...
import {getNumberOfRooms, getRooms,getRoom,updateRoom,createRoom} from "../services/backend/room.service";
...
export async function handleGetOrganizationRooms(req:Request,res:Response){
    ...
    const [rooms,totalRooms]=await Promise.all([getRooms(query,organizationUuid,userUuid),getNumberOfRooms(query,organizationUuid,userUuid)])
    query.offset=(query?.page-1)*query?.limit
    const baseUrl=req.originalUrl?.split("?")[0]
    const responseResult:QueryResponse=new QueryResponse(rooms,totalRooms,baseUrl,query?.page,query?.limit)
